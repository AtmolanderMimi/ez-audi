...
pub trait AudioCodecTrait {
    fn bytes_to_u8_samples(&self, _bytes: &Vec<u8>, _metadata: impl Into<SampleMetadata>) -> Error<Samples<u8>> {
        Err(PlayError::Unsupported("Bytes to u8 samples is not supported for the audio codec".to_string()))
    }

    fn bytes_to_i16_samples(&self, _bytes: &Vec<u8>, _metadata: impl Into<SampleMetadata>) -> Error<Samples<i16>> {
